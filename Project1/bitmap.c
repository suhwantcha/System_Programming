...
/*     추가 구현      */
struct bitmap *bitmap_expand(struct bitmap *bm, size_t exSize) {
    if (bm == NULL || exSize <= 0) {
        return NULL;  // 유효하지 않은 입력이면 NULL 리턴
    }

    // 오버플로우 체크
    if (bm->bit_cnt > SIZE_MAX - exSize) {
        return NULL;
    }
    size_t new_bit_cnt = bm->bit_cnt + exSize;
    size_t old_elem_cnt = elem_cnt(bm->bit_cnt);
    size_t new_elem_cnt = elem_cnt(new_bit_cnt);

    // 메모리 재할당
    elem_type *new_bits = realloc(bm->bits, sizeof(elem_type) * new_elem_cnt);
    if (new_bits == NULL) {
        return NULL;  // 할당 실패하면 NULL 리턴
    }

    // 추가된 부분을 0으로 초기화
    if (new_elem_cnt > old_elem_cnt) {
        memset(&new_bits[old_elem_cnt], 0, (new_elem_cnt - old_elem_cnt) * sizeof(elem_type));
    }

    // 비트맵 업데이트
    bm->bits = new_bits;
    bm->bit_cnt = new_bit_cnt;
    return bm;
}